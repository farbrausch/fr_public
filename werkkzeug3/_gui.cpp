...
//    r.And(p->Position);
  ...
  if(SplashMat)
  {
    ...
    sRect r;
    r.x0 = si.XSize/2-SplashSizeX/2;
    r.y0 = si.YSize/2-SplashSizeY/2;
    r.x1 = r.x0+SplashSizeX;
    r.y1 = r.y0+SplashSizeY;
    sPainter->Flush();
    sPainter->Paint(SplashMat,r,~0);
  }
}
...
void sGuiManager::OnFrame()
{
  ...
  for(i=0;i<PostIndex;i++)
  {
    sInt cmd = PostBuffer[i];         // this helps a bug when WM_PAINT is called from inside WM_PAINT
    PostBuffer[i] = 0;
    if(cmd)
...
//    sGui->Palette[sGC_BUTTON]  = 0xff737373;
    ...
    sGui->Palette[sGC_SELBACK] = 0xffc85858; // a8a8a8
...
void sGuiManager::SetSplashScreen(sBitmap *bm)
{
  SplashMat = sPainter->LoadBitmapAlpha(bm);
  SplashSizeX = bm->XSize;
  SplashSizeY = bm->YSize;
...
void sGuiWindow::RemBorder(sGuiWindow *win)
{
  sGuiWindow **p;
...
void sToolBorder::OnPaint()
{
  ...
  if((sGui->GetStyle() & sGS_SKIN05) && !MenuStyle)
  {
    ...
    if(w && w->GetClass()==sCID_VSPLITFRAME)
    {
      sInt hoverOffset = (Flags & sGWF_HOVER) ? 24 : 0;

      sRect rr;
      if(w->GetChild(0)!=ww)
      {
        rr = r;  rr.x1 = rr.x0+4;
        uv.x0 = (69 + hoverOffset)/128.0f;
        uv.x1 = (73 + hoverOffset)/128.0f;
        uv.y0 =  0/128.0f;
        uv.y1 = 14/128.0f;
        sPainter->Paint(sGui->SkinMat,rr,uv,~0);
      ...
      {
        rr = r;  rr.x0 = rr.x1-4;
        uv.x0 = (64 + hoverOffset)/128.0f;
        uv.x1 = (68 + hoverOffset)/128.0f;
        uv.y0 =  0/128.0f;
        uv.y1 = 14/128.0f;
        sPainter->Paint(sGui->SkinMat,rr,uv,~0);
  ...
  // handle layouting of context menu (if present)
  sGuiWindow *p = Childs;
  sInt extraState = 0;
  sInt selectExtra = (sGui->GetStyle() & sGS_SKIN05) ? 0 : 1;

  while(p)
  {
    if(p->Flags & 0x80000000)
    {
      sU32 newFlags = p->Flags;

      if(extraState == selectExtra)
        newFlags &= ~sGWF_DISABLED;
      else
        newFlags |= sGWF_DISABLED;
...
void sToolBorder::AddContextMenu(sU32 cmd,sBool right)
{
  // basically a hack: add both variants (with image and with "v"),
  // then (later) dynamically disable the right one in OnLayout

  sImageButton *img;
  img = new sImageButton;
  img->InitContext();
  img->Cmd = cmd;
  img->Flags = 0x80000000 | sGWF_DISABLED;
  if(right)
    img->Flags |= sGWF_OTHERSIDE;
  AddChild(img);
  ...
  con->Menu("V",cmd,0);
  con->Style |= sCS_DONTCLEARBACK;
  con->Flags |= 0x80000000 | sGWF_DISABLED;
  AddChild(con);
...
void sControl::FormatValue(sChar *out,sInt zone)
{
  ...
  if(Type==sCT_FLOAT)
  {
    ...
    sSPrintF(out,32,buffer,DataF[zone]*UnitScale[zone]);
  }
  if(Type==sCT_HEX)
  {
  ...
  if(Type==sCT_FIXED)
  {
    ...
    sSPrintF(buffer,sizeof(buffer),"%%.%df",i);
    sSPrintF(out,32,buffer,DataS[zone]*UnitScale[zone]/65536.0f);
  }
  if(Type==sCT_MASK)
  {
    out[0] = '0';
  ...
  case sKEY_APPFOCLOST:
    if(Style & sCS_EDITNUM)
    {
      ...
      case sCT_FIXED:
        ...
        if(*s==0)
        {
          DataS[DragZone] = sRange<sInt>(fval/UnitScale[DragZone]*0x10000,Max[DragZone],Min[DragZone]);
          Post(DoneCmd);
        }
        break;
      case sCT_INT:
      ...
      case sCT_FLOAT:
        ...
        if(*s==0)
        {
          DataF[DragZone] = sRange<sF32>(fval/UnitScale[DragZone],Max[DragZone],Min[DragZone]);
          Post(DoneCmd);
        }
        break;
      case sCT_FRGBA:
...
sBool sListControl::OnCommand(sU32 cmd)
{
  ...
  case sCMD_LIST_MOVEDOWN:
    ...
    return sTRUE;
  case sCMD_LIST_MOVELEFT:
    OnKey(sKEY_LEFT|sKEYQ_CTRL);
    return sTRUE;
  case sCMD_LIST_MOVERIGHT:
    OnKey(sKEY_RIGHT|sKEYQ_CTRL);
    return sTRUE;
...
void sListControl::ClearSelect(sBool selected)
{
  sInt i;
...
void sTextControl::CmdCut()
{
  ...
  if(SelMode && s0!=s1)
  {
    sSystem->SetClipboard(TextBuffer->Text+s0,len);
    Engine(s0,len,0);
    SelMode = 0;
    Cursor = s0;
  }
...
#define CP_CHANGERGB      0x100
#define CP_CHANGEHSV      0x101
#define CP_EXIT           0x102

static sF32 Mod1(sF32 f)
{
  f = sFMod(f,1);
  if(f<0) f+=1;
  return f;
...
static void IntHSV(sU8 *dest,sInt h,sInt s,sInt v)
{
	sInt i;
	sInt f, p, q, t;
  sU8 *r = dest+2;
...
sColorPickerWindow::sColorPickerWindow(sU32 cmd,sU8 *data,sBool alpha,sGuiWindow *send)
{
  sControl *con;
  sInt Line;
  ...
  HSVBitmap = new sBitmap;
  HSVBitmap->Init(128,128);
  sTexInfo ti;
  ti.Init(HSVBitmap);
  HSVTex = sSystem->AddTexture(ti);
  sVERIFY(sISHANDLE(HSVTex));

  HSVMat = sPainter->LoadMaterial(new sSimpleMaterial(HSVTex,sMBF_BLENDALPHA,0,0),HSVBitmap->XSize,HSVBitmap->YSize);


  AddTitle("Pick Color",0,1);

  Line = 0;
  Grid = new sGridFrame;
  Grid->SetGrid(2,99,0,sPainter->GetHeight(sGui->PropFont)+2);  
  Grid->Add(new sMenuSpacerControl("RGB"),0,2,Line++);

  Grid->AddLabel("Red",0,1,Line);
  con = new sControl;  con->EditFloat(CP_CHANGERGB,&Red,0);  con->InitNum(0,255,1,0);
  con->LayoutInfo.Init(1,Line,2,Line+1);    Grid->AddChild(con);   Line++;

  Grid->AddLabel("Green",0,1,Line);
  con = new sControl;  con->EditFloat(CP_CHANGERGB,&Green,0);  con->InitNum(0,255,1,0);
  con->LayoutInfo.Init(1,Line,2,Line+1);    Grid->AddChild(con);   Line++;

  Grid->AddLabel("Blue",0,1,Line);
  con = new sControl;  con->EditFloat(CP_CHANGERGB,&Blue,0);  con->InitNum(0,255,1,0);
  con->LayoutInfo.Init(1,Line,2,Line+1);    Grid->AddChild(con);   Line++;

  if(AlphaEnable)
  {
    Grid->AddLabel("Alpha",0,1,Line);
    con = new sControl;  con->EditFloat(CP_CHANGERGB,&Alpha,0);  con->InitNum(0,255,1,0);
    con->LayoutInfo.Init(1,Line,2,Line+1);    Grid->AddChild(con);   Line++;
  }

  Grid->Add(new sMenuSpacerControl("HSV"),0,2,Line++);

  Grid->AddLabel("Hue",0,1,Line);
  con = new sControl;  con->EditFloat(CP_CHANGEHSV,&Hue,0);  con->InitNum(-360,720,1.0f,0);
  con->LayoutInfo.Init(1,Line,2,Line+1);    Grid->AddChild(con);   Line++;

  Grid->AddLabel("Saturation",0,1,Line);
  con = new sControl;  con->EditFloat(CP_CHANGEHSV,&Sat,0);  con->InitNum(0,1,0.005f,0);
  con->LayoutInfo.Init(1,Line,2,Line+1);    Grid->AddChild(con);   Line++;

  Grid->AddLabel("Value",0,1,Line);
  con = new sControl;  con->EditFloat(CP_CHANGEHSV,&Value,0);  con->InitNum(0,1,0.005f,0);
  con->LayoutInfo.Init(1,Line,2,Line+1);    Grid->AddChild(con);   Line++;
...
void sColorPickerWindow::ChangeRGB()
{
  ...
  b = Blue/255.0f;
	...
	min = sF32(sMin(r,sMin(g,b)));
	max = sF32(sMax(r,sMax(g,b)));
  Value = max;
...
void sColorPickerWindow::UpdateOutput()
{
  ...
  {
    if(SendTo)
      sGui->Post(ChangeCmd,SendTo);
    else
      sGui->Post(ChangeCmd,Parent);
  }
}

void sColorPickerWindow::OnPaint()
{
  sRect r;
  sInt cr = sRange(sInt(Red  ),255,0);
  sInt cg = sRange(sInt(Green),255,0);
  sInt cb = sRange(sInt(Blue ),255,0);

  if(PickValue!=Value)
  {
    sInt v = sRange(sInt(Value*255),255,0);
    for(sInt y=0;y<HSVBitmap->YSize;y++)
    {
      sU8 *d = (sU8*)(&HSVBitmap->Data[y*HSVBitmap->YSize]);
      for(sInt x=0;x<HSVBitmap->XSize;x++)
      {
        IntHSV(d,x*6*256/HSVBitmap->XSize,y*256/HSVBitmap->YSize,v);
        d[3]=255;
        d+=4;
      }
    }
    sSystem->UpdateTexture(HSVTex,HSVBitmap);
    PickValue = Value;
  }

  r.Init(Client.x0,BarRect.y1,Client.x1,BarRect.y1+1);
  sPainter->Paint(sGui->FlatMat,r,sGui->Palette[sGC_DRAW]);  
  r.Init(PickRect.x0-1,PickRect.y0,PickRect.x0,PickRect.y1);
  sPainter->Paint(sGui->FlatMat,r,sGui->Palette[sGC_DRAW]);  
  r.Init(PickRect.x1,PickRect.y0,PickRect.x1+1,PickRect.y1);
  sPainter->Paint(sGui->FlatMat,r,sGui->Palette[sGC_DRAW]);  

  sPainter->Paint(sGui->FlatMat,BarRect,0xff000000|(cr<<16)|(cg<<8)|cb);

  sGuiPainterVert v[4];
  v[0].Init(GrayRect.x0,GrayRect.y0,0xffffffff,0,0);
  v[1].Init(GrayRect.x1,GrayRect.y0,0xffffffff,0,0);
  v[2].Init(GrayRect.x1,GrayRect.y1,0xff000000,0,0);
  v[3].Init(GrayRect.x0,GrayRect.y1,0xff000000,0,0);
  sPainter->Paint(sGui->FlatMat,v,1);
  r = GrayRect;
  r.y0 = GrayRect.y0 + sInt((GrayRect.YSize()-2)*(1-Value));
  r.y1 = r.y0+2;
  sPainter->Paint(sGui->FlatMat,r,0xffff0000);//(Value>0.5)?0xff000000 : 0xffffffff);


  sInt x = sInt(Mod1(Hue/360)*PickRect.XSize())+PickRect.x0;
  sInt y = sInt(Sat*PickRect.YSize())+PickRect.y0;
  sInt n = 3;
  sInt color = Value>0.5 ? 0xff000000 : 0xffffffff;

  sPainter->Paint(HSVMat,PickRect,0xffffffff);

  sGui->Clip(PickRect);
  sPainter->Line(x-n,y  ,x  ,y-n,color);
  sPainter->Line(x  ,y-n,x+n,y  ,color);
  sPainter->Line(x+n,y  ,x  ,y+n,color);
  sPainter->Line(x  ,y+n,x-n,y  ,color);

  if(!(Flags & (sGWF_CHILDFOCUS|sGWF_HOVER)))
    sGui->Post(CP_EXIT,this);
}
...
void sColorPickerWindow::OnLayout()
{
  ...
  Grid->Position.y0 = y;
  BarRect.y1 = y-1;
  Grid->Position.x1 = x-1;
  PickRect.x0 = x;
  PickRect.y0 = y;
  GrayRect = PickRect;
  GrayRect.x0 = GrayRect.x1-20;
  PickRect.x1 = GrayRect.x0-1;
}

void sColorPickerWindow::OnDrag(sDragData &dd)
{
  switch(dd.Mode)
  {
  case sDD_START:
    ...
    if(PickRect.Hit(dd.MouseX,dd.MouseY))
      DragMode = 1;
    if(GrayRect.Hit(dd.MouseX,dd.MouseY))
      DragMode = 2;
    break;
  case sDD_DRAG:
    if(DragMode==1)
    {
      Hue = sFMod((dd.MouseX-PickRect.x0)*360.0f/PickRect.XSize(),360);
      Sat = sRange<sF32>((dd.MouseY-PickRect.y0)*1.0f/PickRect.YSize(),1.0f,0.0f);
      ChangeHSV();
    }
    if(DragMode==2)
    {
      Value = 1-sRange<sF32>((dd.MouseY-GrayRect.y0)*1.0f/(GrayRect.YSize()-2),1.0f,0.0f);
      ChangeHSV();
    }
    break;
  case sDD_STOP:
...
sBool sColorPickerWindow::OnCommand(sU32 cmd)
{
  switch(cmd)
  {
  case CP_CHANGERGB:
...
void sColorPickerWindow::OnKey(sU32 key)
{
  switch(key&0x8001ffff)
  {
  case sKEY_ESCAPE:
