...
// calculate current object
  ...
  const sChar *beattickstr = "bar";
  sInt beattickint = (beat>>16)+1;
  if(1)//App->Doc->TimelineMode)
  {
    beattickstr = "beat";
    beattickint = beat>>14;
  }
  ...
  {
    sF32 t = sDivShift(beat-App->MusicLoop0,App->MusicLoop1-App->MusicLoop0)/65536.0f;
    App->Doc->KEnv->Var[0].Init4(t,t,t,t);

    sSPrintF(buffer,sizeof(buffer),"%s %04d time %02d:%02d.%02d mark %d%%",beattickstr,beattickint,ms/1000/60,ms/1000%60,ms%1000/10,sInt(t*100));
    App->SetStat(STAT_TIME,buffer);
  }
  else
  {
    sSPrintF(buffer,sizeof(buffer),"%s %04d time %02d:%02d.%02d",beattickstr,beattickint,ms/1000/60,ms/1000%60,ms%1000/10);
    App->SetStat(STAT_TIME,buffer);
  }
  ...
  if(ShowObject)
  {
    ...
    if(ShowObject->Error==0)
    {
      ...
      App->Doc->KEnv->DebugOpByte = App->AnimPageWin->GetDebugCode();
      App->Doc->KEnv->DebugOpOutput.Init(0,0,0,0);
      sSystem->ProgressGDIStart(App->Doc->CountOps(ShowObject));
...
//  kenv.Var[0].Init(0,0,0,0);
  ...
  if(Flags&sGWF_CHILDFOCUS)
  {
    ...
    else
    {
      ...
      {
        if(cid==KC_BITMAP)
          App->SetStat(STAT_DRAG,"l:normalmap light   m:scroll [menu]   r:zoom");
        else
      ...
      else if(qual&sKEYQ_SHIFT)
      {
        if(cid==KC_BITMAP)
          ...
          App->SetStat(STAT_DRAG,"l:edit xz   m:edit y [menu]   r:edit rot");
      }
      else
      {
        if(cid==KC_BITMAP)
...
// calculate sized viewport
  ...
  if(ShowOrigSize && (cid==KC_MESH || DragCID==KC_MINMESH || cid==KC_SCENE || cid==KC_EFFECT || cid==KC_IPP || cid==KC_DEMO))
  {
    sInt xs,ys;
    sRect r;
...
// performance counters
  ...
  static sInt timer_last;
  static sF32 timer_smooth;

  sInt timer_delta = sSystem->GetTime()-timer_last;
  timer_last += timer_delta;
  timer_smooth = (7*timer_smooth+1*timer_delta)/8;

  sSPrintF(buffer,sizeof(buffer),"%07.3f fps / %07.3f ms, %d tri, %d line, %d vert, %d batches, calc %4d ms",1000.0f/timer_smooth,timer_smooth,StatWireTri,StatWireLine,StatWireVert,Perf.Batches,time);
...
*/
  case 't':                       // bitmap: tile 
    ...
    else if(DragCID==KC_IPP || DragCID==KC_DEMO || DragCID==KC_SCENE || DragCID==KC_MINMESH || DragCID==KC_MESH || DragCID==KC_EFFECT)
    {
      if(!App->TextureMode)
  ...
  case 't'|sKEYQ_BREAK: 
    if(!App->TextureMode)
      if(DragCID==KC_IPP || DragCID==KC_DEMO || DragCID==KC_SCENE)
...
//  sMatrix mat;
  ...
  case sDD_START:
    ...
    if(DragCID == KC_BITMAP)
    {
      ...
      case 2:
        DragMode = DMB_ZOOM;
        CalcShowSize(xs,ys);
        DragStartX0 = sDivShift(dd.MouseX-BitmapX-Client.x0,xs);
        DragStartY0 = sDivShift(dd.MouseY-BitmapY-Client.y0,ys);
        DragStartX = dd.MouseX;
        DragStartY = dd.MouseY;
        DragStartZ = BitmapZoom;
        break;
      default:
        DragMode = DMB_SCROLL;
    ...
    if(DragCID == KC_SCENE || DragCID == KC_MESH || DragCID == KC_MINMESH || DragCID == KC_IPP || DragCID == KC_SMESH || DragCID == KC_DEMO || DragCID == KC_MATERIAL || DragCID == KC_EFFECT)
    {
      ...
      case 0x43:
        ...
        case 0x102:  // cube
          rp = DragEditOp->Op.GetEditPtrF( 7); 
          tp = DragEditOp->Op.GetEditPtrF(10); 
          break;
        case 0x113:  // select cube
          tp = DragEditOp->Op.GetEditPtrF(1); 
          break;
        case 0x120:  // transform
          rp = DragEditOp->Op.GetEditPtrF( 4); 
          tp = DragEditOp->Op.GetEditPtrF( 7); 
          break;
        case 0x121:  // transform ex
          rp = DragEditOp->Op.GetEditPtrF( 4); 
          tp = DragEditOp->Op.GetEditPtrF( 7); 
          break;
  ...
  case sDD_DRAG:
    ...
    case DMB_ZOOM:   // zoom bitmap
      BitmapZoom = sRange<sInt>(DragStartZ - (-dd.DeltaX+dd.DeltaY)*0.02,8,0);
      CalcShowSize(xs,ys);
      BitmapX = DragStartX - sMulShift(xs,DragStartX0) - Client.x0;
      BitmapY = DragStartY - sMulShift(ys,DragStartY0) - Client.y0;
      break;
...
sBool WinView::OnCommand(sU32 cmd)
{
  ...
  {
  case CMD_VIEW_POPUP:
  case CMD_VIEW_PULLDOWN:
    mf = new sMenuFrame;
    switch(DragCID)
    {
    case KC_SCENE:
      mf->AddMenu("Background Color",CMD_VIEW_COLOR,'c');
      ...
      mf->AddMenu("align to grid normal",CMD_VIEW_90DEGREE,'i');
  ...
  */
      ...
      mf->AddSpacer();
      mf->AddMenu("Help",CMD_VIEW_HELP_DEFAULT,0);
      break;
    case KC_MESH:
    case KC_MINMESH:
      mf->AddMenu("Background Color",CMD_VIEW_COLOR,'c');
  ...
  //      mf->AddMenu("Shadow/Light",CMD_VIEW_SHADOWTOG,'y');
      ...
      mf->AddSpacer();
      mf->AddMenu("Help",CMD_VIEW_HELP_MESH,0);
      break;
    case KC_BITMAP:
      mf->AddMenu("Reset",CMD_VIEW_RESET,'r');
      mf->AddMenu("Copy to Clipboard",CMD_VIEW_EXPORTBITMAP,sKEY_ENTER);
      mf->AddMenu("Background Color",CMD_VIEW_COLOR,'c');
      mf->AddSpacer();
      mf->AddCheck("Tile",CMD_VIEW_TILE,'t',BitmapTile);
      mf->AddCheck("Alpha",CMD_VIEW_ALPHA,'a',BitmapAlpha);
      mf->AddSpacer();
      mf->AddMenu("Help",CMD_VIEW_HELP_BITMAP,0);
      break;
    case KC_IPP:
    case KC_DEMO:
      mf->AddMenu("Reset",CMD_VIEW_RESET,'r');
...
//      mf->AddMenu("Shadow/Light",CMD_VIEW_SHADOWTOG,'y');
      ...
      mf->AddMenu("next cam",CMD_VIEW_NEXTCAM,'+');
      mf->AddMenu("prev cam",CMD_VIEW_PREVCAM,'-');
      mf->AddMenu("Viewport -> Free Camera",CMD_VIEW_GOFREECAMERA,'Z'|sKEYQ_SHIFT);
      mf->AddCheck("Free Camera",CMD_VIEW_FREECAMERA,'z',GenOverlayManager->FreeCamera);
      mf->AddSpacer();
      mf->AddMenu("Help",CMD_VIEW_HELP_DEFAULT,0);
      break;
    case KC_MATERIAL:
      ...
      mf->AddSpacer();
      mf->AddMenu("Help",CMD_VIEW_HELP_MATERIAL,0);
      break;
    default:
      mf->AddMenu("Help",CMD_VIEW_HELP_DEFAULT,0);
      break;
  ...
  case CMD_VIEW_HELP_DEFAULT:
    App->Help("viewwindow_default");
    return sTRUE;
  case CMD_VIEW_HELP_BITMAP:
    App->Help("viewwindow_texture");
    return sTRUE;
  case CMD_VIEW_HELP_MESH:
    App->Help("viewwindow_mesh");
    return sTRUE;
  case CMD_VIEW_HELP_MATERIAL:
    App->Help("viewwindow_material");
    return sTRUE;
  ...
  case CMD_VIEW_RESET:
    ...
    BitmapTile = 0;
    BitmapAlpha = 0;
    BitmapClearColor = sGui->Palette[sGC_BACK];
    MeshWire = 0;
    SceneWire = 0;
    SceneLight = -1;
    SceneUsage = -1;
...
//    ShowOrigSize = 0;
  ...
  case CMD_VIEW_GOFREECAMERA:
    GenOverlayManager->FreeCamera = 1;
    GenOverlayManager->LinkEdit = 0;
    Cam.State[3] = App->Doc->KEnv->CameraDataRot.x;
    Cam.State[4] = App->Doc->KEnv->CameraDataRot.y;
    Cam.State[5] = App->Doc->KEnv->CameraDataRot.z;
    Cam.State[6] = App->Doc->KEnv->CameraDataPos.x;
    Cam.State[7] = App->Doc->KEnv->CameraDataPos.y;
    Cam.State[8] = App->Doc->KEnv->CameraDataPos.z;
    Cam.Zoom[0] = Cam.Zoom[1] = 128.0f/App->Doc->KEnv->CameraDataZoom;
    return sTRUE;
  ...
  case CMD_VIEW_COLOR:
    con = new sControl;
    if(DragCID==KC_BITMAP)
      con->EditURGB(0,(sInt *)&BitmapClearColor,0);
    else
  ...
  case CMD_VIEW_ALPHA:
    ...
    return sTRUE;
  case CMD_VIEW_ORTHO:
    if(DragCID==KC_MESH || DragCID == KC_MINMESH || DragCID==KC_SCENE || DragCID==KC_IPP || DragCID==KC_DEMO || DragCID==KC_EFFECT)
      Cam.Ortho = !Cam.Ortho;
    return sTRUE;
  case CMD_VIEW_GRID:
    if(DragCID==KC_MESH || DragCID == KC_MINMESH || DragCID==KC_SCENE || DragCID==KC_IPP || DragCID==KC_DEMO || DragCID==KC_EFFECT)
...
void WinView::CalcShowSize(sInt &xs,sInt &ys)
{
  if(LastTexSizeX >= LastTexSizeY)
  {
    xs = 256 * (1<<BitmapZoom)/16;
    ys = sMulDiv(xs,LastTexSizeY,LastTexSizeX);
  }
  else
  {
    ys = 256 * (1<<BitmapZoom)/16;
    xs = sMulDiv(ys,LastTexSizeX,LastTexSizeY);
  }
  if(xs<1) xs = 1;
  if(ys<1) ys = 1;
}
...
void WinView::ShowBitmap(sViewport &view,class GenBitmap *tex)
{
  ...
  if(tex)
  {
    ...
    sSystem->SetViewport(view);
    sSystem->Clear(sVCF_ALL,BitmapClearColor & 0xffffff);
    ...
    LastTexSizeX = sMax(tex->XSize,1);
    LastTexSizeY = sMax(tex->YSize,1);
    CalcShowSize(xs,ys);
    ...
    handle = sSystem->GeoAdd(sFVF_TSPACE3BIG,sGEO_TRISTRIP|sGEO_DYNAMIC);
...
void WinView::ShowScene(sViewport &view,WerkOp *op,KEnvironment *kenv)
{
  ...
  {
    sZONE(ExecTree);
    if(op->Op.Cache && op->Op.Cache->ClassId == KC_EFFECT)
      Engine->AddPaintJob(&op->Op,kenv->ExecStack.Top(),kenv);
    else
      op->Op.Exec(kenv);
  }
...
void WinView::ExportBitmap(GenBitmap *gbm)
{
  ExportBitmapFlag = 0;

  sBitmap *bm = new sBitmap();
  bm->Init(gbm->XSize,gbm->YSize);
  sU8 *d = (sU8 *)bm->Data;
  sU16 *s = (sU16 *)gbm->Data;
  for(sInt i=0;i<gbm->XSize*gbm->YSize;i++)
  {
    d[0] = sRange(s[0]>>7,255,0);
    d[1] = sRange(s[1]>>7,255,0);
    d[2] = sRange(s[2]>>7,255,0);
    d[3] = sRange(s[3]>>7,255,0);
    d+=4;
    s+=4;
  }
  sSystem->SetClipboard(bm);
}
