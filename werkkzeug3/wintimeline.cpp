...
sBool WinTimeline::OnCommand(sU32 cmd)
{
  ...
  case CMD_TIME_POPUP:
    ...
    mf->AddMenu("Bypass",CMD_TIME_BYPASS,'b');
...
void WinTimeline::OnPaint()
{
  ...
  for(i=0;i<App->Doc->SoundEnd>>16;i+=skip)
  {
    ...
    if((i&15)==0)
    {
      sSPrintF(buffer,sizeof(buffer),"%d",App->Doc->TimelineMode ? i*4 : i+1);
...
void WinTimeline::OnCalcSize()
{
  ...
  if(SizeX<BLOCKONE)
    SizeX = BLOCKONE;
  SizeY = BLOCKONE*64;
  sDPrintF("calcsize: %08x %08x -> %08x\n",App->Doc->SoundEnd,BeatZoom,SizeX);
}
...
void WinEvent::SetEvent(WerkEvent *we)
{
  ...
  if(we)
  {
    ...
    Label("Disable Event");
    con = new sControl;
    con->EditCycle(0,&we->Disabled,0,"Enabled|Disabled");
    Grid->Add(con,3,5,Line++);

    Label("Flags");
    con = new sControl;
    con->EditCycle(0,&we->Event.Flags,0,"|Preserve Time");
    Grid->Add(con,3,5,Line++);
