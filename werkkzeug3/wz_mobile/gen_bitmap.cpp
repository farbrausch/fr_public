...
{
  GenBitmap *bm;
  xs = sRange(xs+GenBitmapTextureSizeOffset,2048,0);
  ys = sRange(ys+GenBitmapTextureSizeOffset,2048,0);
  bm = new GenBitmap;
  bm->Init(xs,ys);
  return bm;
}
...
void __stdcall Bitmap_Inner64(sU64 *d,sU64 *s,sInt count,sInt mode,sU64 *x)
{
...
//  SCRIPTVERIFY(b0->ClassId==KC_BITMAP);
  SCRIPTVERIFY(mode>=0 && mode<8);
  if(mode==5) mode=BI_BRIGHTNESS;  // ***SIZE
  if(mode==6) mode=BI_HARDLIGHT;
  if(mode==7) mode=BI_OVER;
...
//    SCRIPTVERIFY(bi->ClassId==KC_BITMAP);
  ...
  while(i<count && (&b0)[i])
  {
    ...
    Bitmap_Inner64(b0->Data,bi->Data,b0->Size,mode,in->Data);
    bi->Release();
    in = b0;
    i++;
  }
...
GenBitmap * __stdcall Bitmap_Color(GenBitmap *bm,sInt mode,sU32 col)
{
  ...
  Bitmap_Inner64(bm->Data,&color,bm->Size,mode+BI_MULCOL,in->Data);
...
GenBitmap * __stdcall Bitmap_Range(GenBitmap *bm,sInt mode,sU32 col0,sU32 col1)
{
  ...
  if(mode&1)
  {
    Bitmap_Inner64(bm->Data,cx,bm->Size,BI_GRAY,in->Data);
    in = bm;
  }
  if(mode&2)
  {
    Bitmap_Inner64(bm->Data,cx,bm->Size,BI_INVERT,in->Data);
    in = bm;
  }
  Bitmap_Inner64(bm->Data,cx,bm->Size,BI_RANGE,in->Data);
...
GenBitmap * __stdcall Bitmap_HSCB(GenBitmap *bm,sF32 fh,sF32 fs,sF32 fc,sF32 fb)
{
  ...
  while(ffh<0)        ffh+=6*65536;
  while(ffh>6*65536)  ffh-=6*65536;
...
//  SCRIPTVERIFY(bc->ClassId==KC_BITMAP);
  ...
  size = bm->Size;
  Bitmap_Inner64(bm->Data,bm->Data,size,BI_GRAY,in->Data);
  switch(mode)
  {
  case 0:
    Bitmap_Inner64(bm->Data,bb->Data,size,BI_MERGE,bc->Data);
    break;
  case 1:
    Bitmap_Inner64(bm->Data,bb->Data,size,BI_MUL,bm->Data);
    Bitmap_Inner64(bm->Data,bc->Data,size,BI_ADD,bm->Data);
    break;
  case 2:
    Bitmap_Inner64(bm->Data,bb->Data,size,BI_MUL,bm->Data);
    Bitmap_Inner64(bm->Data,bc->Data,size,BI_SUBR,bm->Data);
    break;
  case 3:
    Bitmap_Inner64(bm->Data,bb->Data,size,BI_MULMERGE,bc->Data);
    break;
  }
...
//  Bitmap_Inner64(bm->Data,0,bm->Size,8);      // gray bitmap
...
GenBitmap * __stdcall Bitmap_Sharpen(GenBitmap *bm,sInt order,sF32 sx,sF32 sy,sF32 amp)
{
  ...
  Bitmap_Inner64(bm->Data,&color,bm->Size,BI_SHARPEN,bx->Data);
...
GenBitmap * __stdcall Bitmap_Unwrap(GenBitmap *bm,sInt mode)
{
  ...
  for(sInt y=0;y<bm->YSize;y++)
  {
    ...
    for(sInt x=0;x<bm->XSize;x++)
    {
      ...
      {
      case 0: // polar2normal
        sFSinCos(fx * sPI2F,u,v);
        u = (u * fy + 1.0f) * 0.5f;
        v = (v * fy + 1.0f) * 0.5f;
        break;

      case 1: // normal2polar
        u = sFATan2(fxc,fyc) / sPI2F;
        if(u<0.0f) u += 1.0f;
