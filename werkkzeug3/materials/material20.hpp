...
{
public:
  void UpdatePara(const sMaterial20Para &para);

protected:
  enum { MaxNSetups = 4 };

  sInt Setup[MaxNSetups];
  sInt Tex[8];
  sInt TexSet[8];
  sInt TexCount;
  ...
  void Compile(sInt index,const sU32 *states,const sU32 *vs,const sU32 *ps);
...
public:
  sMaterial20Tex(const sMaterial20Para &para,sInt *tex,sBool useAmbient);
