...
#define AOU_EASE          6                 // parameters for ease function 2x(0..1) mapped to float
#define AOU_TIMESHIFT     7                 // parameters for timeshift function 2x(0..1) mapped to int
...
public:
  ...
  sInt ByteCodePtr;                         // offset into generated bytecode, used for debugging
  union
  {
    sVector ConstVector;
    sU32 ConstColor;
...
//  WerkOp *Outputs[KK_MAXOUTPUT];            // duplicated from KOp, use KOp.OutputCount
  ...
  WerkOp *TempLink;                         // used by extract
...
//  void Dump(WerkOp *op,sInt indent=0);
//  void DumpLink(WerkOp *op,sChar *ind,sChar *label,sInt index);
  void ExtractOps(WerkOp *op,WerkPage *clip,const sChar *name);
...
public:
  ...
  sBool ExportTextures(WerkDoc *doc,sU8 *&data,sInt rootMode=0);
};
...
class WinMergePages : public sGuiWindow
{
  sListControl *DocList;
  sControl *Buttons[5];
...
public:
  WinMergePages(WerkkzeugApp *app, WerkDoc *mdoc);
  ~WinMergePages();
...
class WerkkzeugApp : public sDummyFrame
{
  ...
  sInt NovaMode;
  sInt SplashMat;                 // material for splash screen
  sInt SplashSizeX;
  sInt SplashSizeY;
  sInt InitialReadDelay;
...
//  WinAnim *AnimWin;
  ...
  sChar FilenameBuffer[4096];
  sChar FilenameBuffer2[4096];    // used for load backup
  sChar MergeFilenameBuffer[4096];
  sChar WinTitle[256];
  sInt TextureMode;
  ...
  sInt DefaultTexSize;            // bits 0-7: x, bits 8-15: y  (default: 0x0909 => 512x512)
  ...
  sBool Backup(sChar *path);
  void Save(const sChar *path);
  sBool Read(sU32 *&);
  sBool Write(sU32 *&);
  sBool Merge(sU32 *&);
  sBool LoadConfig();
  sBool SaveConfig();
  sBool MakeFilename(sChar *buffer,const sChar *ext,sInt count);
  void SetWindowTitle();
  void Splashscreen();
  void Clear();
  ...
  // help system
  void Help(const sChar *);

  // texture exporting
  sBool ExportBitmaps(sChar *dirName);
  void ExportTextures(sChar *name);
  ...
  sInt GetMode();
  void InitToolBorder();                    // (re-)initialize the tool border
  void MusicReset();
