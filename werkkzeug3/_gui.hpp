...
public:
  ...
  void RemBorder(sGuiWindow *);   // remove a border
...
#define sGWF_OTHERSIDE    0x00004000    // used for sToolborder childs, position them to the right
...
public:
  ...
  void AddContextMenu(sU32 cmd,sBool right=0);
};
...
public:
  ...
  sF32 UnitScale[4];              // scale factor if different units are used. currently only for sCT_FIXED/FLOAT
...
public:
  ...
  sInt GetCount();
  sInt GetSelect();
  void ClearSelect(sBool selected=sFALSE);
...
#define sCMD_LIST_MOVELEFT  0x0109
#define sCMD_LIST_MOVERIGHT 0x010a
...
class sColorPickerWindow : public sGuiWindow
{
  sGridFrame *Grid;
  sGuiWindow *SendTo;
  sU8 *UPtr;
  sF32 Red,Green,Blue,Alpha;
  sF32 Hue,Sat,Value;
  sBool AlphaEnable;
  sU32 ChangeCmd;
...
//  sRect WarpRect;
  ...
  sInt HSVMat;
public:
  sColorPickerWindow(sU32 cmd,sU8 *data,sBool alpha,sGuiWindow *send);
  ~sColorPickerWindow();
  void Tag();
  void ChangeRGB();
  void ChangeHSV();
