...
sInt sAppMain(sInt argc,sChar **argv)
{
  ...
  sSystem->PrintF(sAPPNAME " " sVERSION " >> radical exe packer (c) f. giesen 2003-2005\n\n");
  ...
  else
  {
    ...
    if(!in)
    {
      sSystem->PrintF("\aerror: couldn't open input file\n");
      return 1;
    }
    ...
    {
    case 0: // good
      sSystem->PrintF(" - using good packer frontend\n");
      pfe = new GoodPackerFrontEnd(&pbe);
      break;

    case 1: // best
      sSystem->PrintF(" - using best packer frontend\n");
    ...
    out = pack.Pack(pfe,in,outSize,gotMap ? &info : 0,packerCallback,0);
    if(out)
    {
      ...
      sSystem->SaveFile(outFileName,out,outSize);
