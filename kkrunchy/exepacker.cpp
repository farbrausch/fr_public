  ...
  for(i=0;i<dir->NameEntries + dir->IDEntries;i++,entry++)
  {
    ...
    if(level == 0) // type level
    {
      ...
      if(entry->NameID == 3 || entry->NameID == 14 || entry->NameID == 24)
      {
        ...
        {
          sSPrintF(WarningsPtr,512,"Manifest present and stored uncompressed.");
    ...
    else // child is a leaf
    {
      ...
      if(compFlag) // this will be compressed, so leave it where it is
      {
...
sU8 *EXEPacker::Pack(PackerFrontEnd *pfe,sU8 *source,sInt &outSize,DebugInfo *info,PackerCallback cb,KKBlobList *blobs)
{
  ...
  if(OPH->CheckSum != 0)
  {
    ...
    sSPrintF(WarningsPtr,512,"Checksum present, clearing.");
    WarningsPtr += sGetStringLen(WarningsPtr) + 1;
  }
