...
struc dsdata
  ...
  .callback  resd 1             ; ptr to the callback function
	...
	.gain      resd 1             ; float: gain
	.fadelen   resd 1             ; length of fade
	.fadepos   resd 1							; pos in fade
...
_dsInit@8:
    ...
    push     eax
    push     dword [esp + 44]            ; second parm (hwnd)
    push     esi
...
.ja:
    ...
    ; volume/fade check
		mov     esi, [ebx + dsdata.fadelen]
		or			esi, esi
		jz      .nofade
		mov     esi, [ebx + dsdata.fadepos]
		cmp     esi, [ebx + dsdata.fadelen]
		jae     .silence
		fild    dword [ebx + dsdata.fadepos]
		push    eax
		shr     eax, 2
		add     esi, eax
		pop     eax
		mov     [ebx + dsdata.fadepos], esi
		fidiv   dword [ebx + dsdata.fadelen]
		fld1 
		fsubrp  st1, st0
		jmp    short .fadeend
.silence
...
_dsFadeOut@4:
    push    ebx
		mov			ebx, globdat
		mov			eax, [esp+8]
		mov     [ebx + dsdata.fadelen], eax
		xor     eax, eax
		mov     [ebx + dsdata.fadepos], eax
		pop			ebx
		ret 4
...
streamdesc:
    dd	streamdesc.size-streamdesc
		dd  DSBCAPS_GETCURRENTPOSITION2|DSBCAPS_GLOBALFOCUS|DSBCAPS_LOCSOFTWARE
