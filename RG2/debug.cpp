...
#pragma comment(lib, "dbghelp.lib")
    ...
    static void dumpStackTrace(sInt baseLevel)
    {
      fr::debugOut("Call stack trace:\n");
      
      sU32 *callerPtr, caller=0;
      __asm mov [callerPtr], ebp;
      
      IMAGEHLP_SYMBOL *symb=(IMAGEHLP_SYMBOL *) malloc(sizeof(IMAGEHLP_SYMBOL)+256);
      symb->SizeOfStruct=sizeof(IMAGEHLP_SYMBOL);
      symb->MaxNameLength=255;
      ...
      while (1)
      {
        ...
        if (!SymGetSymFromAddr(GetCurrentProcess(), caller, &disp, symb) && caller)
          break;

        if (caller>=symb->Address+symb->Size)
          strcpy(symb->Name, "[Unknown]");

        if (!baseLevel)
          fr::debugOut("%s <- ", symb->Name);
        else
          baseLevel--;

        if (IsBadReadPtr(callerPtr, 8))
          break;

        caller=callerPtr[1];
        callerPtr=*((sU32 **) *callerPtr);
      }
...
static LONG __stdcall unhandledExceptionFilter(EXCEPTION_POINTERS *exceptionInfo)
{
  ...
  fr::debugOut("\ncontext: EAX=%08x EBX=%08x ECX=%08x EDX=%08x ESI=%08x EDI=%08x EBP=%08x ESP=%08x\n", ctx->Eax, ctx->Ebx,
...
namespace fr
{
	...
	void __cdecl errorExit(const sChar *format, ...)
	{
	  va_list    arg;
    sChar      errbuf[4096];
		static sBool	inErrorProc = sFALSE;
...
static void __cdecl autoDebugInit(void)
{
...
static void __cdecl autoDebugClose(void)
{
  processExitProcs();
