...
; sample rate dependent float constants 
...
oscjtab dd syOscRender.off, syOscRender.mode0, syOscRender.mode1, syOscRender.mode2, 
				dd syOscRender.mode3, syOscRender.mode4, syOscRender.off, syOscRender.off
...
.m4loop1
      ...
      fmul  dword [fcfmmax]
      fmul  dword [fc32bit]      ; -2^31 .. 2^31
      fistp dword [ebp + syWOsc.tmp]
      add   ebx,  [ebp + syWOsc.tmp]
			shr   ebx, 9
			add   eax, edx
			or    ebx, 0x3f800000							
			mov   [ebp + syWOsc.tmp], ebx
...
syDRMTab		dd syDistRenderMono.mode0, syDistRenderMono.mode1, syDistRenderMono.mode2, syDistRenderMono.mode3
            ...
            dd syDistRenderMono.modeF, syDistRenderMono.modeF
...
.mode4 ; decimator
		mov   eax, [ebp + syWDist.dfreq]
		mov   ebx, [ebp + syWDist.dcount]
.m4loop
;		fld   dword [ebp + syWDist.dlp2bl]    ; <lp2>
;		fld   dword [ebp + syWDist.dlp1bl]    ; <lp1> <lp2>
;		fsub  st0, st1                        ; <in-lp1> <lp1> <lp2>
;		fmul  dword [ebp + syWDist.dlp1c]     ; <c1(in-lp1> <lp1> <lp2>
;		faddp st1, st0                        ; <lp1'> <lp2>
;		fst   dword [ebp + syWDist.dlp1bl]  

		add   ebx, eax
		jnc   .m4nonv
		fld   dword [esi]                     ; <in> <lp1> <lp2>
		fstp  dword [ebp + syWDist.dvall]     ; <dval> <lp2>
.m4nonv
		fld   dword [ebp + syWDist.dvall]     ; <dval> <lp2>
		lea   esi, [esi + 4]

;		fsub  st0, st1                        ; <dval-lp2>
;		fmul  dword [ebp + syWDist.dlp2c]     ; <c1(dval-lp2> <lp2>
;		faddp st1, st0                        ; <lp2'>
;		fst   dword [ebp + syWDist.dlp2bl]  
...
syDRSTab		dd syDistRenderMono.mode0, syDistRenderMono.mode1,syDistRenderMono.mode2, syDistRenderMono.mode3
            ...
            dd syDistRenderStereo.modeF, syDistRenderStereo.modeF
...
.m4loop
		mov   eax, [ebp + syWDist.dfreq]
		add   [ebp + syWDist.dcount], eax
		jnc   .m4nonv
		fld   dword [esi]
		fstp  dword [ebp + syWDist.dvall]
		fld   dword [esi+4]
		fstp  dword [ebp + syWDist.dvalr]
.m4nonv
		lea   esi, [esi + 8]
		fld   dword [ebp + syWDist.dvall]
		fstp  dword [edi]
		fld   dword [ebp + syWDist.dvalr]
		fstp  dword [edi+4]
		lea   edi, [edi + 8]
		dec   ecx
	jnz .m4loop
