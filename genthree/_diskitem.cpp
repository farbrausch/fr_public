...
public:
  ...
  sInt Cmd(sInt cmd,sChar *s,sDiskItem *d);
...
sInt sDIRoot::Cmd(sInt cmd,sChar *s,sDiskItem *d)
{
  return sFALSE;
}
...
sInt sDIFile::Cmd(sInt cmd,sChar *s,sDiskItem *d)
{
  sChar path[sDI_PATHSIZE];
  sInt result;
...
sInt sDIDir::Cmd(sInt cmd,sChar *name,sDiskItem *di)
{
  sInt result;
...
sInt sDIFolder::Cmd(sInt cmd,sChar *s,sDiskItem *d)
{
  return 0;
}
...
sInt sDIApp::Cmd(sInt cmd,sChar *name,sDiskItem *di)
{
  if(cmd==sDIC_EXECUTE)
  {
    if(Start)
...
sInt sDISystemInfo::Cmd(sInt cmd,sChar *s,sDiskItem *d)
{
  switch(cmd)
  {
  case sDIC_CUSTOMWINDOW:
#if sLINK_GUI
    return (sInt) new sSystemInfoWindow;
#else
    return sFALSE;
#endif
  default:
