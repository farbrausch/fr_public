...
sTYPEINFO(sCID_OGGDECODER,sCID_SOUNDFILTER,sOGGDecoder);

sOGGDecoder::sOGGDecoder()
{
/*
  Info.Caps = sSIC_16 | sSIC_MOVE;
  Info.Channels = 2;
  Info.Freq = 44100;
  Info.Gran = 1;
  Info.MinSize = 1;
...
void sOGGDecoder::Load(sU8 *data,sInt size,sBool del)
{
  if(DeleteStream && Stub->Coded)
    delete[] Stub->Coded;
...
sBool sOGGDecoder::Load(sChar *name)
{
  sInt size;
  sU8 *data;
  sBool result;
...
void sOGGDecoder::Handler(void *stream,sInt samples,sU32 time)
{
  sInt count;
  sU32 *d;

  d = (sU32 *) stream;
//  sVERIFY(info.SampleSize()==4);
...
/****************************************************************************/
/*
sPtr sOGGDecoder::Render(sInt &size)
{
  sU8 *data;
  sU8 *copy;
  sInt max;
  ...
  size = 0;
  i = dec->DecodeFrame(Buffer);
  while(i!=0 && size+i<max)
  {
    sCopyMem(data+size,Buffer,i);
    size+=i;
    i = dec->DecodeFrame(Buffer);
  }

  copy = new sU8[size];
  sCopyMem(copy,data,size);
  delete[] data;
